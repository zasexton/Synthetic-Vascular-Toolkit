...
wd = os.getcwd()
d  = wd+os.sep+'benchmarking'+os.sep+'optimization'
if not os.path.isdir(d):
    os.mkdir(d)

get_data = """import pickle
import glob
import os
import numpy as np
...
plt.show()
"""
file = open(d+os.sep+"get_data.py","w+")
file.write(get_data)
file.close()

for sol in solvers:
    if not os.path.isdir(d+os.sep+sol):
        os.mkdir(d+os.sep+sol)
    for i in range(10):
        t = svcco.tree()
        t.convex=True
        t.set_boundary(s)
        t.set_root()
        t.n_add(100,method=sol)
        filename = d + os.sep + sol + os.sep + 'tree_cube_{}'.format(i)
        file = open(filename,'wb+')
        pickle.dump(t.time,file)
        file.close()
